...
export const useAdminProjectsStore = () => {
  ...
  async function fetch(search: string, page: number, signal: AbortSignal) {
    pending.value = true;
  ...
  async function update(
    update: (
      pagination: Pagination<ProjectResponse>,
    ) => Pagination<ProjectResponse>,
  ) {
    projects.value = update({
      ...projects.value,
    } as Pagination<ProjectResponse>);
  }
