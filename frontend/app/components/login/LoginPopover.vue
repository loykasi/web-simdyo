...
<template>
  <UPopover
    ...
    arrow
  >
    <UButton :label="$t('nav.login')" color="neutral" variant="ghost" />
    <template #content>
      <LoginOTPForm
        v-if="isOpenLoginForm"
        :email="email"
        @back-to-request-form="backToRequestForm"
      />
      <RequestOTPForm v-else :email="email" @to-login-form="toLoginForm" />
    </template>
  </UPopover>
</template>
